...
welcomeMessage() {
    echo -e "${GREEN}###################################################"
    echo -e "Welcome to the SLURM Installation Script for Ubuntu 24.04 (ARM64)"
...
checkArchitecture() {
    ...
    if [ "$ARCH" != "aarch64" ]; then
        ...
        echo "Use install.sh for x86_64 systems."
        exit 1
    fi
    echo "Architecture: $ARCH (ARM64) confirmed."
}
...
installDependencies() {
    ...
    # Only install libnvidia-ml-dev if NVIDIA GPU is present
    if command -v nvidia-smi &>/dev/null; then
        sudo apt install -y libnvidia-ml-dev || true
    fi
}

setupRngTools() {
    echo "Setting up RNG tools..."
    sudo rngd -r /dev/urandom || true
}

setupMunge() {
    echo "Setting up Munge..."
    sudo /usr/sbin/mungekey -f || true
